...
namespace load_layout_data_node
{
    ...
    inline bool in_seglines(const std::vector<std::pair<std::string, std::string>> &list, std::pair<std::string, std::string> q_name)
    {
        ...
        if (list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                if (((q_name.first == list[i].first) && (q_name.second == list[i].second)) || ((q_name.first == list[i].second) && (q_name.second == list[i].first))) {
                    in_list = true;
                }
            }
        }
    ...
    Load_Layout_Data_Node::Load_Layout_Data_Node(ros::NodeHandle nh)
    {
        ...
        std::string robot_pose_topic, cleaner_path_topic, stop_trig_topic, arm_turn_topic, solar_add_height_topic, cleaner_targrt_finish_topic, cleaner_mission_targrt_topic, arm_trig_topic;
        ...
        are_to_map = vec_matrix(v_are_to_map);                                                               // 从参数服务器获取变换矩阵
        std::vector<double> station_param;
        nh_param.param<std::vector<double>>(are_heard + "/station", station_param, std::vector<double>()); // 从参数服务器获取station参数

        XmlRpc::XmlRpcValue param_list_are;

        if (nh_param.getParam(are_heard + "/node_list", param_list_are) == true) { // are_leapting/node_list
            ...
            for (int i = 0; i < (int)param_list_are.size(); i++) {
                ...
                temp_pose[2] = 0.0;
                temp_pose[3] = 1.0;
                result_pose = are_to_map * temp_pose;
        ...
        if (block_name.size() > 0) {
            for (int m = 0; m < (int)(block_name.size()); m++) {
                ...
                if (nh_param.getParam(param_heard + "/solar_row_list", param_list) == true) {
                    ...
                    for (int i = 0; i < param_list.size(); i++) {
                        ...
                        std::vector<Solar_Session> session_value;
                        ...
                        for (int j = 0; j < param_size; j = j + 5) {
                            ...
                            pt.y = result_pose[1];
                            positionCloud->push_back(pt);
                            session_value_chip.height = param_list[i]["param"][j + 4];
                            session_value.push_back(session_value_chip);
                        }
                        std::string full_row_name = block_name[m] + "/" + row_name;
                        std::pair<std::string, std::vector<Solar_Session>> map_data_chip(full_row_name, session_value);
                        row_size++;
                    ...
                    //         std::cout << "row_start_point: " << iter->second[i].start_point.first << ", " << iter->second[i].start_point.second << std::endl;
    ...
    bool find_seg(const std::vector<std::pair<std::string, std::string>> &seg_list, const std::vector<road_node> &node_list,
                  std::pair<double, double> pa, std::pair<double, double> pb, std::pair<road_node, road_node> &seg_line)
    {
    ...
    bool find_seg(const std::vector<std::pair<std::string, std::string>> &seg_list, const std::vector<road_node> &node_list,
                  std::pair<double, double> pa, std::pair<road_node, road_node> &seg_line)
    {
        std::vector<std::pair<road_node, road_node>> closet_node;
        for (int i = 0; i < seg_list.size(); i++) {
            road_node road_node1, road_node2;
    ...
    void Load_Layout_Data_Node::target_trig_callback(const std_msgs::HeaderConstPtr &msg)
    {
        ...
        try {
            target_block_name = results[0];
            target_row_name = results[1];
            results_number = splitString(results[1], "_");
            target_row_header = results_number[0];
            target_start_row_number = stringToNum<int>(results_number[1]);
        ...
        if (results[1] != "0_0") {
            ...
            int ten_number = msg->seq % 100;
            if ((ten_number / 10) == 0) { // default row_up mode
                ROS_INFO("From row up!");
                target_row_size = block_row_size - target_start_row_number + 1;
                ...
                target_row_size = target_start_row_number;
                std::string row_name_chip;
                for (int i = 0; i < target_row_size; i++) {
                    row_name_chip = target_block_name + "/" + target_row_header + "_" + std::to_string(target_start_row_number - i);
                    row_names.push_back(row_name_chip);
                ...
                int end_row_number = (msg->seq / 100);
                if (end_row_number <= 0) {
                    end_row_number = 1;
                }
                target_row_size = abs(end_row_number - target_start_row_number) + 1;
                std::string row_name_chip;
            ...
            for (int i = 0; i < target_row_size; i++) {
                std::cout << "row_names[i] " << row_names[i] << std::endl;
                iter = map_data.find(row_names[i]);
                if (iter != map_data.end()) {
                    std::pair<double, double> start_p_raw, end_p_raw, start_p, end_p;

                    if (flag_s_e == false) {
                        start_p_raw = iter->second[iter->second.size() - 1].end_point;
                        end_p_raw = iter->second[0].start_point;
                        ...
                        for (j = 0; j < iter->second.size(); j++) {
                            ...
                            gap_action_chip1.gap_point = extension_line(iter->second[iter->second.size() - 1 - j].end_point, iter->second[iter->second.size() - 1 - j].start_point, cleaner_width - brake_dis, false);
                            gap_action_chip1.up_down = 2;
                            ...
                            gap_action_chip2.gap_point = extension_line(iter->second[iter->second.size() - 1 - j].start_point, iter->second[iter->second.size() - 1 - j].end_point, cleaner_width + brake_dis, false);
                            gap_action_chip2.up_down = 1;
                        ...
                        if ((i == (target_row_size - 1) && (j == iter->second.size()))) {
                            gap_action_chip3.up_down = 0;
                        } else {
                            gap_action_chip3.up_down = 3;
                        }
                        ...
                        for (j = 0; j < iter->second.size(); j++) {
                            gap_action gap_action_chip1;
                            gap_action_chip1.gap_point = extension_line(iter->second[j].start_point, iter->second[j].end_point, cleaner_width - brake_dis, false);
                            gap_action_chip1.up_down = 2;
                            ...
                            row_gap_actions.push_back(gap_action_chip1);
                            gap_action gap_action_chip2;
                            gap_action_chip2.gap_point = extension_line(iter->second[j].end_point, iter->second[j].start_point, cleaner_width + brake_dis, false);
                            gap_action_chip2.up_down = 1;
            ...
            if (path_init) {
                ...
                global_gap_actions.clear();
                if (false == find_seg(segline_names, road_nodes, std::pair<double, double>(target_path.poses[0].pose.position.x, target_path.poses[0].pose.position.y),
                ...
                // std::cout << "turn_seg.first.node_name " << turn_seg.first.node_name << std::endl;
                // std::cout << "turn_seg.second.node_name " << turn_seg.second.node_name << std::endl;
                road_node end_node = point_dis(turn_seg.first.position, base_node.position) > point_dis(turn_seg.second.position, base_node.position) ? turn_seg.first : turn_seg.second;
            ...
            if (path_init) {
                ...
                for (int i = 0; i < solar_path_size - 1; i++) {
                    if (i == 0) {
                        std::pair<double, double> g1(global_path.poses[global_path_size - 1].pose.position.x, global_path.poses[global_path_size - 1].pose.position.y);
                        std::pair<double, double> g2(global_path.poses[global_path_size - 2].pose.position.x, global_path.poses[global_path_size - 2].pose.position.y);
            ...
            if (false == find_seg(segline_names, road_nodes, std::pair<double, double>(curr_robot_pose.position.x, curr_robot_pose.position.y), nearest_seg)) {
                ROS_ERROR_STREAM("Can not find global segment line near current robot pose: " << curr_robot_pose.position);
                return;
            }
            road_node back_start_node = point_dis(nearest_seg.first.position, station_in.position) > point_dis(nearest_seg.second.position, station_in.position) ? nearest_seg.first : nearest_seg.second;
            ...
            if (find_path(back_start_node, station_in, back_node_list)) {
                ...
                std::pair<double, double> cross_p = GetFootOfPerpendicular(std::pair<double, double>(curr_robot_pose.position.x, curr_robot_pose.position.y), a, b, c);
